...
class Convert extends React.Component {
  ...
  render() {
    ...
    return (
      <Card title={intl.get('convert.title')}>
        ...
        <Form layout="horizontal">
          <Form.Item label={intl.get('convert.amount')} colon={false} {...formItemLayout} className="mb0" extra={
            <div className="row">
              <div className="col-auto">{priceValue}</div>
              <div className="col"></div>
              <div className="col-auto"><a href="" onClick={selectMax.bind(this)}>{intl.get('convert.max')}</a></div>
            </div>
          }>
            {form.getFieldDecorator('amount', {
              initialValue: '',
              rules: [
                {message: intl.get('convert.amount_verification_message'), transform:(value)=>fm.toNumber(value),
          ...
          <Form.Item className="mb0 mt15">
            {this.state.selectMaxWarn &&
              <div className="fs12 color-grey-500 text-center mb5">
                {intl.get('convert.min_gas_remain_warn')}
              </div>
            }
            {this.state.inputMaxWarn &&
              <div className="fs12 color-grey-500 text-center mb5">
                {intl.get('convert.no_eth_balance_warn')}
              </div>
            }
            <Button onClick={handleSubmit.bind(this)} type="primary" className="d-block w-100" size="large">{intl.get('convert.confirm_convert')}</Button>
            {this.state.errorMsg &&
