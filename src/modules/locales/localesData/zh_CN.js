export default {
  ...
  trades_faq_a1:`
    可能原因1：您的订单的 LRC Fee 过小：矿工的撮合收益（LRC Fee）大于 撮合成本（以太交易的油费）, 导致矿工不撮合您的订单。
    <br />
    可能原因2：您的订单没有找到匹配的买单/卖单
    <br />
    可能原因3：您的订单已经被撮合了，并且已经被提交到以太网络，但是以太网络非常拥堵，导致交易无法确认（几分钟~几十分钟不等，甚至更长时间）
    <br />
  `,
  trades_faq_q2:"Q2 : 从下单到交易完成需要多长时间？",
  trades_faq_a2:`
    交易撮合时间（矿工挖掘环路）：如果存在匹配的买单和卖单，你的订单很快就会被矿工撮合（几秒以内）。
    <br />
    交易提交时间（矿工提交环路）：矿工会把撮合成功的交易提交到以太网络，提交时间受以太网络拥堵情况的限制，从十几秒到几十分钟不等。
    <br />
    从上面内容可知：交易完成的时间最快十几秒，慢则几十分钟甚至更长，以太网络的拥堵情况起着决定性的因素。
  `,
  trades_faq_q3:"Q3 : 网站维护期间，我的币怎么提出来？",
  trades_faq_a3:`
    路印协议不托管你的任何数字资产，交易前无需充值，交易后无需提现。
    <br />
    即使进行了交易挂单，您的所有的以太数字资产也一直在您的以太地址里。
    <br />
    网站维护期间，您可以通过任意的第三方钱包查看和管理您的数字资产。
  `,
